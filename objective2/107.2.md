...
* arquivos de crontab onde o usuário é especificado geralmente estão em `/etc/crontab` ou `/etc/cron.d`.
* `cron.d` arquivos específico de crontab onde o usuário deve ser definido.
...
* podem ser especificado valores para as variáveis `PATH`, `SHELL`, `MAILTO`, etc.
...
* no debian o diretório `crontab` armazena os arquivos de crontab dos usuário.
